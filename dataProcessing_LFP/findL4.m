function ops=findL4(eLFP,CSD,MUA,ops)
...
%     plot(t_min,V_min,'r','LineWidth',2)
    ...
    hold on
    plot(CSD.Source/ops.fs*1000-1000,(1:16),'b')
    plot(CSD.Sink/ops.fs*1000-1000,(1:16),'r')
    hold off
    ...
    if size(L4channelN,2)==1
        ops.L4=[L4channelN.*2-1,L4channelN.*2];
        ops.L4=[min(ops.L4)-1 ops.L4 max(ops.L4)+1];
        ops.L4=sort(ops.L4,'ascend');
    elseif size(L4channelN,2)==2 || size(L4channelN,2)==3 
        ops.L4=[L4channelN.*2-1,L4channelN.*2];
        ops.L4=sort(ops.L4,'ascend');
    else
    ...
    if ops.verbose
        ...
        plot([-50 1050],[min(ops.L4)*size(CSD.interp,1)/ops.NchanTOT min(ops.L4)*size(CSD.interp,1)/ops.NchanTOT],'k:','LineWidth',1)
        plot([-50 1050],[max(ops.L4)*size(CSD.interp,1)/ops.NchanTOT max(ops.L4)*size(CSD.interp,1)/ops.NchanTOT],'k:','LineWidth',1)
        text(-45,max(ops.L4)*size(CSD.interp,1)/ops.NchanTOT-50,'L4','FontSize',20)
