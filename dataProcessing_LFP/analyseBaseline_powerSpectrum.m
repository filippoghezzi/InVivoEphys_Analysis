function spectral=analyseBaseline_powerSpectrum(ops,s,varargin)
...
% Output: ops -> struct, containing function outputs.
    ...
    LFP_PSD=zeros(ops.NchanTOT,numel(LFP_frequency));
    MUA = get_eLFP(ops.fbinary,1,ops.fs,ops.NchanTOT,[0 60*5],'MUA');
    ...
    if plotFlag
        ...
        sg=sgtitle(strcat('Baseline analysis - ',ops.binaryRoot(15:end),' - P',num2str(ops.age)));
        sg.FontSize=30;

        ax=subplot(2,3,1:2);
        PSDplot=plot(LFP_PSD_f,mean(LFP_PSD(ops.L4,:)));
        PSDplot.LineWidth=2;
        PSDplot.Color='k';
